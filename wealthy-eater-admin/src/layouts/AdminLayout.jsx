...
export default function AdminLayout() {
  ...
  // 🔥 Đồng bộ hiển thị tiêu đề chính xác trên Topbar theo Design System
  const getTopbarTitle = () => {
    switch(location.pathname) {
      ...
      case '/nutritionists': return 'Nutritionists Directory'; // UC-84 Target Title
      case '/recipes': return 'Recipes Management';
      default: return 'Admin Panel';
    }
  };

  // 🔌 Cấu trúc Menu Sidebar chuẩn xác
  const navItems = [
    ...
    { path: '/nutritionists', icon: UserCheck, label: 'Manage Nutritionists' }, // Kiểm duyệt hồ sơ & chứng chỉ chuyên gia y tế
    ...
    <div className="flex min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <aside className="w-64 fixed inset-y-0 left-0 bg-white border-r border-slate-200 z-50 flex flex-col shadow-sm">
        <div className="h-16 flex items-center px-6 border-b border-slate-100">
          <UtensilsCrossed className="w-6 h-6 text-emerald-600 mr-3" /> {/* Sử dụng tone màu chính Emerald làm chủ đạo */}
              ...
              <NavLink 
                ...
                className={({ isActive }) => 
                  `flex items-center px-3 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 ${
                    isActive 
                      ? 'bg-emerald-50 text-emerald-700 font-bold shadow-sm border-l-4 border-emerald-600 rounded-l-none' 
        ...
        <div className="p-4 border-t border-slate-100">
          <button 
            onClick={logout} 
            className="flex items-center w-full px-3 py-2.5 rounded-lg text-sm font-medium text-red-600 hover:bg-red-50 transition-colors duration-200 border-none cursor-pointer"
          >
