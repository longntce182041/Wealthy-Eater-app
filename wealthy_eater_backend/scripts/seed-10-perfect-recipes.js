...
async function seed10PerfectRecipes() {
  try {
    await mongoose.connect(process.env.MONGODB_URI);
    console.log('Connected to MongoDB.');

    // 1. Create Micronutrients
    const micros = [
    ...
    // 2. Define the 10 Ultimate Recipes Data
    const recipesData = [
      {
    ...
    for (const rData of recipesData) {
      ...
      console.log(`✅ Fully populated: ${rData.name}`);
    }

    console.log('\n🎉 Successfully seeded 10 completely perfect relational recipes!');
    process.exit(0);
  } catch (err) {
    console.error(err);
    process.exit(1);
